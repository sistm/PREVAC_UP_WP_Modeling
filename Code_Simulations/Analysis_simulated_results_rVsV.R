# ----------------------- #
# OBJECTIVE: Code analyzing results generated by the file "Simulation_results_rVsV.R", for the vaccine strategy rVsV
...
Folder_results <- "Rcode/Results/rVSV_2"   # Folder in which results were saved
Nb_simulation <- 2000
...
for(n in 1:Nb_simulation){
  # n <- 1
  if(n %% 100 == 0){print(n)}
  results <- read.table(file=paste(Folder_results,paste("rVSV_Time_Simu",n,".csv",sep=""),sep="/"),header=TRUE,sep="\t",dec=".")  
...
Distribution_estimated_time <- ddply(.data=Estimated_time,.variables = .(ARM,CATAGE,SEX,Ab_thresh),summarise,
                                     Mean=mean(Time+28,na.rm=TRUE),
                                     Median=median(Time+28,na.rm=TRUE),
                                     # 95% Confidence interval
                                     ICMIN=quantile(Time+28,probs=c(0.025),na.rm=TRUE),
                                     ICMAX=quantile(Time+28,probs=c(0.975),na.rm=TRUE))
...
Sex_labels <- c("Men","Women")
names(Sex_labels) <- c(0,1)
...
Plot_estimated_Time <- ggplot(data=subset(Distribution_estimated_time,ARM==1)) +
  ...
  facet_rep_wrap(SEX~.,repeat.tick.labels = TRUE,ncol=1,labeller=labeller(SEX=Sex_labels)) +
  
  scale_x_continuous(name="Antibody anti GP-EBOV concentration threshold",breaks=color_axis$x) +
  scale_y_continuous(name="Time from vaccination required for 50% of the population \ndropping below antibody threshold (in days)",breaks=seq(0,4000,by=250)) + 
  ...
  coord_cartesian(ylim=c(0,3250),xlim=c(100,2520),expand = 0) +
...
ggsave(Plot_estimated_Time,filename = paste("Data","rVsV_Time_Vs_AbThres_2000simu_7500Subj.png",sep="/"),height = 8,width=12)
...
# -- Information about results generated by the file "Simulation_results_rvsv.R" ####
Folder_results <- "Rcode/Results/rVSV_2"   # Folder iin which results were saved
Nb_simulation <- 2000
Ab_thresholds <- seq(100,5000,by=100)
...
for(n in 1:Nb_simulation){
  # n <- 1
  if(n %% 100 == 0){print(n)}
  tmp_results <- read.table(file=paste(Folder_results,paste("rVSV_proba_Simu",n,".csv",sep=""),sep="/"),header=TRUE,sep="\t",dec=".")  
...
# Calculation of distributions
Distribution_estimated_proba <- ddply(.data=Estimated_Proba,.variables = .(ARM,CATAGE,SEX,Ab_thresh),summarise,
...
color_axis$color[which(color_axis$x %in% c(200,600,1000))] <- "red"
Sex_labels <- c("Men","Women")
names(Sex_labels) <- c(1,2)

Plot_estimated_Proba <- ggplot(data=subset(Distribution_estimated_proba,ARM==1)) +
  geom_vline(xintercept = c(200,600,1000),color="black",size=0.75,linetype="dashed") +
  ...
  theme(panel.background = element_rect(fill="white"),
        ...
        legend.box = "vertical",
        plot.title = element_text(size=13,hjust=0.5))
ggsave(Plot_estimated_Proba,filename = paste("Data","rVsV_Proba_Vs_AbThres_2000simu_7500Subj.png",sep="/"),height = 8,width=12)
