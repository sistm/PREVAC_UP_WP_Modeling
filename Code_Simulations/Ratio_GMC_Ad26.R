...
dfprevac <- dfprevac[,c("SUBJID_N", "VISIT","COUNTRYID", "ARM", "SEX", "CATAGE", "ABBn","TEST_MED_FINAL_LOG","VISITa","VISITb")]
colnames(dfprevac) <- c("IDs","TIME","COUNTRYID", "ARM", "SEX", "CATAGE", "ABBn","LOG10AB","TIMEa","TIMEb")
dfprevac$TIMEb[which(dfprevac$TIME == 365 & dfprevac$TIMEb == 180)] <- 185

Ad26_participants <- unique(dfprevac$IDs)
Ad26_Nbpart <- length(Ad26_participants)
# ----- #
...
clusterExport(Cluster,c("Bootstrap_dataset_creation","dfprevac","Ad26_Nbpart","Ratios_GMT_Estimation"))
...
Bootstraps_ratios <- foreach(n=seq(1,Nb_bootstrap),.combine="rbind",.errorhandling = "stop",.packages = c("parallel","foreach","doSNOW","plyr","lme4","psych"),.options.snow = opts)%dopar%{
  ...
  boot_MEM_Ad26 <- lmer(data = boot_dataset,
                        formula = LOG10AB ~ TIMEa*ARM*CATAGE + TIMEa*ARM*COUNTRYID + TIMEa*ARM*ABBn + TIMEb*ARM*CATAGE + TIMEb*ARM*COUNTRYID + TIMEb*ARM*ABBn +  (1 | IDs_BOOT), REML = T)
  
  # -- Addition of model predictions in the bootstrap dataset
  boot_dataset$LOG10PRED <- predict(boot_MEM_Ad26)
  ...
  # Ratios Age category
  Ratios_CATAGE <- rbind(cbind(Boot=n,Time=0,Ratios_GMT_Estimation(data=subset(boot_dataset,TIME == 90),targeted_variable="CATAGE",reference="Adults")),
                         cbind(Boot=n,Time=275,Ratios_GMT_Estimation(data=subset(boot_dataset,TIME == 365),targeted_variable="CATAGE",reference="Adults")))
  
  # Ratios Country 
  Ratios_COUNTRY <- rbind(cbind(Boot=n,Time=0,Ratios_GMT_Estimation(data=subset(boot_dataset,TIME == 90),targeted_variable="COUNTRYID",reference="Mali")),
                          cbind(Boot=n,Time=275,Ratios_GMT_Estimation(data=subset(boot_dataset,TIME == 365),targeted_variable="COUNTRYID",reference="Mali")))
  
  # Ratios Pre-vaccine antibody concentration
  Ratios_ABBn <- rbind(cbind(Boot=n,Time=0,Ratios_GMT_Estimation(data=subset(boot_dataset,TIME == 90),targeted_variable="ABBn",reference=0)),
                       cbind(Boot=n,Time=275,Ratios_GMT_Estimation(data=subset(boot_dataset,TIME == 365),targeted_variable="ABBn",reference=0)))
  Ratios_ABBn$Category <- as.character(Ratios_ABBn$Category) 
  
  Results_Ratio <- rbind(Ratios_CATAGE,Ratios_COUNTRY,Ratios_ABBn)
